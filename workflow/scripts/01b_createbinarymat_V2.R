...
#query <- suppressMessages(readBed(mergedPeakFile))     # this line doesn't work properly
# ------- manually load in consensus peak as GR:
query <- fread(mergedPeakFile, data.table = FALSE)
query <- query[,c(1:6)]
colnames(query) <- c("chr", "start", "end", "width", "strand", "score")
